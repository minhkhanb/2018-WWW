...
public class TapdaqManager{
	...
	static public void init(Activity activity){
		...
		config.registerTestDevices(TMMediationNetworks.FACEBOOK, Arrays.asList("5aed4475e520da214ef84e88166bcdaa"));
		...
		enabledPlacements.add(TapdaqPlacement.createPlacement(Arrays.asList(CreativeType.INTERSTITIAL_LANDSCAPE, CreativeType.INTERSTITIAL_PORTRAIT), "default"));
	...
	static public void onInitSuccess()
	{
		...
		Tapdaq.getInstance().loadInterstitial(CurrentActivity, "default", _interstitial_listener);
	...
	static public boolean isPrepareInterstitialCompleted(){
		return Tapdaq.getInstance().isInterstitialReady(CurrentActivity, "default");
	}
	...
	static public boolean showInterstitial(ActionArg arg){
		...
		{
			_lst_waiting_args.add(arg);
			Tapdaq.getInstance().showInterstitial(CurrentActivity, "default", _interstitial_listener);
			return true;
		}
		else {
			arg.onCancel();
