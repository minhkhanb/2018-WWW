...
PineUnityAdsAsync::PineUnityAdsAsync()
{
	...
	_count_frame_for_prepare_rewarded_video_ads = -1;
}
	...
	if (_svc_prepare_video.Status == PineServiceResult::WaitingResponse){
		...
		//	showInterstitial();
			...
			else
			{
				...
				{
					//prepareVideo();
					PDEBUG("Unity ads video prepare not done wait %d for recheck!", COUNT_FOR_PREPARE_UNITY_REWARDED_VIDEO_ADS);
					_count_frame_for_prepare_rewarded_video_ads = COUNT_FOR_PREPARE_UNITY_REWARDED_VIDEO_ADS;
				}
			}
		}
	}
