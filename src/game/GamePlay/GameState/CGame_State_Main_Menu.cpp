...
{
	PDEBUG("\n TouchButtonMainMenu => TouchButtonMainMenu:%d", GAME()->_topebox_game.render_banner_topebox);
	bool pressed = false;
#if defined (ANDROID_OS)
	if (GAME()->_topebox_game.render_banner_topebox)
	{
		return true;
	}
#endif
	//PDEBUG("\n _quest_popup_active:%d -- _is_ggplay_menu_active:%d", _quest_popup_active, _is_ggplay_menu_active);
	if (_is_ggplay_menu_active || _quest_popup_active || _rating_active   /*|| GAME()->_topebox_game.show_banner_topebox_active*/)
	{
		return true;
	}
	if (!ProcessingButton())
...
bool WildWildWestGame::CQuest::UpdateTouch() {
	...
	if (ret)
	{
		...
		if (_quest_button[index].NeedAction())
		{
			...
			case BUTTON_QUEST_X2REWARD:
				//game->Logs_DailyQuest_Double(true);
				//Show unity ads when get reward
#if !ADS_DISABLE
#if defined (MAC_OS)
			{
				...
				if (Device::IsNetworkAvailable())
				{
					...
					//game->_count_delay_loading = TIME_DELAY_LOADING;
				}
#else
				_reward_quest = _reward_quest * 2;
#endif
...
bool WildWildWestGame::ProcessingButton()
{
	...
	if (_rating_active || GAME()->_topebox_game.render_banner_topebox)
	{
		return true;
	}
