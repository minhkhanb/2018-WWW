...
#endif
	PDEBUG("\n _is_waiting_for_reward:%d -- _on_video_ads_reward:%d -- game->_show_ads_interrupt:%d", _is_waiting_for_reward, _on_video_ads_reward, game->_show_ads_interrupt);
}
...
void CGame::UpdateTouch(int xx, int yy, int touch_state, int touch_id, int tap_count)
{
	...
	//PDEBUG("\n _current_state => %d", game->_current_state);
...
void CGame::ShowBannerAds()
{
	...
	if (_tapdaq_manager.isBannerReady() && !_tapdaq_manager.isShowBanner())
	{
		...
		game->_has_show_ads_banner = true;
	}
	else {
		game->_time_count_for_show_ads_banner = 2 * FPS_MAX;
	}
#endif
}
...
void CGame::UpdateAds()
{
	...
	if (OS_IsNetworkAvailable())
	{
		...
		case PineTapdaqAsync::enumTapdaqUpdateResult::SHOW_VIDEO_SUCCESS:
			...
			{
				game->_time_delay_show_ads = TIME_DELAY_SHOW_ADS * 60;
				_on_video_ads_reward = true;
				game->_isAdsProcessing = false;
			}
			PDEBUG("\n Show Tapdaq success SHOW_VIDEO_SUCCESS");
			if (MAIN_GAME()->_current_state == k_GS_PLAYGAME)
			{
				MAIN_GAME()->SetState(k_GS_IGM);
				CGame::_this->_isBackkeyPress = false;
