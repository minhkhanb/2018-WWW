...
void WildWildWestGame::Init()
{
	...
	_count_delay_loading = TIME_DELAY_LOADING;
...
void WildWildWestGame::UpdateAndRender()
{
	...
	//PDEBUG("\n _purchase_iap_id:%d", _purchase_iap_id);
	...
	if (_current_state >= k_GS_LOGO)
	{
		...
		if (_count_delay_loading > 0) {
			_count_delay_loading--;
			if (_count_delay_loading <= 0)
			{
				_count_delay_loading = 0;
	...
	if (_current_state > k_GS_LOGO)
	{
		...
		if (!_user._removeAdd)
		{
			...
			if (_count_frame_show_ads > 0)
			{
				...
				if (_count_frame_show_ads <= 0)
				{
					...
					{
						//Show Tapdaq ads after Topebox ads popup inactive
						GAME()->ShowAds();
					}
				}
			}
		...
		if (_current_state == k_GS_PLAYGAME)
		{
			if (_count_frame_delay_show_ads > 0)
			{
				...
				//Show ads:
				//+ After fire Play button
				//+ After 2.5 minutes
				PDEBUG("\n _time_delay_show_ads:%d [-] _count_frame_delay_show_ads:%d [-] _countShowAdsMods:%d [-]  _countShowAdsUnity:%d", _time_delay_show_ads, _count_frame_delay_show_ads, _countShowAdsMods, _countShowAdsUnity);
				if (!_user._removeAdd && _time_delay_show_ads <= 0 && _count_frame_delay_show_ads <= 0)
				{
...
#if !DISBALE_TOPEBOX_ADS
		if (GAME()->configDownloaded && !_user._removeAdd)
		{
